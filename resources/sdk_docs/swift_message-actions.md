...
Use message actions to add/remove metadata on published messages (receipts, reactions, custom data). Clients subscribe to a channel to receive action events in real time and can fetch past actions from Message Persistence.

- Reactions: Specific use of Message Actions for emoji/social reactions.
...
- type actionType (String): Message action type.
- value (String): Message action value.
- messageTimetoken (Timetoken): Timetoken of the target message.
- custom requestConfig (PubNub.RequestConfiguration = PubNub.RequestConfiguration()): Per-request configuration. See Request Configuration.
- completion ((Result<PubNubMessageAction, Error>) -> Void)? = nil: Async result of the call.
...
- custom requestConfig (RequestConfiguration = RequestConfiguration()): Per-request configuration.
- completion ((Result<(channel: String, message: Timetoken, action: Timetoken), Error>) -> Void)? = nil: Async result of the call.

### Completion handler result

#### Success

Tuple containing the channel, message Timetoken, and action Timetoken for the removed action.
...
Truncated response: If results are truncated, a more property is returned with paging parameters. Make iterative calls using the provided paging values.
...
- page (PubNubBoundedPage? = PubNubBoundedPageBase()): Paging object with start, end, and limit.
...
An Array of PubNubMessageAction for the channel, and the next request PubNubBoundedPage (if present).
