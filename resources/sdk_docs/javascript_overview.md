...
##### Chat SDK
Use the Chat SDK (TypeScript, built on this JavaScript SDK) for chat features like users, channels, messages, typing indicators, quotes, mentions, and threads: /docs/chat/chat-sdk
...
## Overview
The JavaScript SDK provides a unified real-time messaging API across:
- Web (browser)
- Node.js (server)
- React Native (mobile)
...
Supported environments: browsers, Node.js, React Native, React, Angular, Vue, and other JS frameworks.
...
- Create an app; use the keyset (publish and subscribe keys)
- Use separate keysets per environment when possible
...
### Initialize PubNub
Create an instance with publishKey, subscribeKey, and a unique userId.
...
- React Native lifecycle example (cleanup)
```
...
Use addListener to handle messages, presence events, and status updates.

For details: /docs/sdks/javascript/api-reference/publish-and-subscribe#subscribe
...
- React Native (full app example is provided in Complete example)
...
- React Native (with presence and cleanup)
```
`1useEffect(() => {  
2    if (!pubnub) return;  
3      
...
Reference: /docs/sdks/javascript/api-reference/publish-and-subscribe#subscribe

### Publish messages
Messages are JSON-serializable and must be < 32 KiB. Publishing delivers to all subscribers of the channel.
...
To use this React Native example:
- Create a project (Expo or RN CLI)
```
...
- SDK API reference: /docs/sdks/javascript/api-reference/configuration
- GitHub: https://github.com/pubnub/javascript
- Tutorials:
