...
const illuminateSafetyRules =
  "Safety rules for this workflow: Before any change, ask the user which account, app, and keyset to use, and ask them to confirm it is a test keyset with no production traffic. publish-fake-data sends real messages, and active Decisions run real actions (publishes, webhooks, App Context membership updates). Until the user approves, only read: list, get, get-fields, raw-snapshot, field-health, aggregate, verify-query, check-action-log. Before each create, update, activate, deactivate, or publish-fake-data call, show the exact change and wait for approval. Never delete a resource unless the user names it; if a limit blocks a create, list the candidates and let the user choose. ";
